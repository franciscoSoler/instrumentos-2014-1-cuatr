...
\section{Desarrollo}
	...
	\subsection{Mediciones con el RLC}		
		...
		\todo[inline]{modificar esta tabla }
		\begin{table}[!htp]
			\centering
			\begin{tabular}{|c|c|c|c|c|}
				\hline
		...
		junto a la ecuación \ref{eq:002} para poder obtener los valores de la 
		reactancia total en las mismas frecuencias en que se realizaron las 
		mediciones con el RLC meter. La tabla \ref{tab:011} muestra los 
		resultados.
		...
		una comparación entre los valores de las reactancias de ambos modelos,
		el RC serie medido con respecto al RLC serie real.

		\todo[inline]{agregar la imagen con ref=img:002}
		...
		calcular los parámetros del capacitor (\ref{eq:003} y \ref{eq004}), se 
		obtiene la tabla \ref{tab:012}.

		\todo[inline]{insertar la tabla que tiene los parametros del capacitor}
		...
		\todo[inline]{insertar la tabla del modulo de la reactancia en frec}

		\indent A modo de comparación se muestra el gráfico \ref{img:004} de las comparaciones
		de las reactancias medida y calculadas en la tabla \ref{tab:013}

		\todo[inline]{insertar en grafico 004 la comparacion}
